...
class TShirtGenerator:
    ...
    def generate_design(self, collection_info: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                'title': collection_info.get('title', 'AI Design'),
    ...
    def create_shopify_product(self, design_info: Dict, printful_product: Dict) -> Optional[str]:
        ...
        # Check for new style (access token)
        if self.shopify_store_name and self.shopify_access_token:
            shopify_configured = True
            use_access_token = True
        # Check for old style (API key + password)
        elif all([self.shopify_api_key, self.shopify_password, self.shopify_store_name]):
            shopify_configured = True
        ...
        try:
            ...
            if use_access_token:
                # New style with access token
                shopify_url = f"https://{self.shopify_store_name}.myshopify.com/admin/api/2024-01/products.json"
                headers = {
                ...
                # Old style with API key/password
    ...
    def generate_daily_designs(self, specific_categories: List[str] = None) -> None:
        ...
        try:
            ...
            for collection_key in categories:
